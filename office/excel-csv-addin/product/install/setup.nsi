...
Section "Install"
  ...
  CopyFiles "$INSTDIR\${PRODUCT_SYMBOL}.xla" "$APPDATA\Microsoft\AddIns\"
  ...
  ExecWait 'wscript.exe "$INSTDIR\addin_enable.vbs" ${PRODUCT_SYMBOL} /i' $ReturnCode
...
Section "Uninstall"
  ...
  Delete /REBOOTOK "$INSTDIR\addin_enable.vbs"
  Delete /REBOOTOK "$INSTDIR\${PRODUCT_SYMBOL}.xla"

  ; アドインファイルの削除
  DetailPrint "### アドインファイルの削除"
  Delete /REBOOTOK "$APPDATA\Microsoft\AddIns\${PRODUCT_SYMBOL}.xla"
